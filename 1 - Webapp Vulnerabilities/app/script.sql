...
IF NOT EXISTS (SELECT * from users WHERE email = 'admin@example.com' AND username = 'Admin' AND password = 'password')
    INSERT INTO users (email, password, name) VALUES ('admin@example.com', 'password', 'Admin');


IF NOT EXISTS (SELECT * from posts WHERE username = 'Admin' AND text = 'Welcome to <blog name>, please behave and stuff')
