...
USE ponyfarm;
CREATE TABLE IF NOT EXISTS users(id INT AUTO_INCREMENT PRIMARY KEY, email VARCHAR(255) UNIQUE, password VARCHAR(255), name VARCHAR(255));
...
WHERE NOT EXISTS (SELECT * from posts
    WHERE username = 'Admin' AND text = 'Welcome to BLORG, please behave and stuff' LIMIT 1);